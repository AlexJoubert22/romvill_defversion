...
#rv-faq .search input::placeholder{color:#B1AEAB}
#rv-faq .search input:focus{border-color:#F0C24A;background:rgba(255,255,255,.12);box-shadow:0 0 0 4px rgba(240,194,74,.14)}
#rv-faq .search .sic{position:absolute;left:17px;top:50%;transform:translateY(-50%);width:20px;height:20px;color:#B1AEAB}
#rv-faq .wrap{max-width:820px;margin:0 auto;padding:56px 20px 20px}
#rv-faq .cat{display:flex;align-items:center;gap:12px;font-weight:800;text-transform:uppercase;letter-spacing:.2em;font-size:.72rem;color:#F0C24A;margin:36px 0 14px}
#rv-faq .cat:first-child{margin-top:0}
#rv-faq .cat span{height:1px;flex:1;background:linear-gradient(90deg,rgba(240,194,74,.5),transparent)}
#rv-faq .item{scroll-margin-top:90px;background:#fff;border:1px solid #e7e9ee;border-radius:16px;margin-bottom:12px;overflow:hidden;box-shadow:0 1px 2px rgba(0, 0, 0,.04);opacity:0;transform:translateY(22px);transition:box-shadow .28s,border-color .28s,transform .6s ease,opacity .6s ease}
.dark #rv-faq .item{background:#000000;border-color:#000000}
#rv-faq .item.reveal{opacity:1;transform:none}
#rv-faq .item:hover{box-shadow:0 10px 30px rgba(0, 0, 0,.09)}
#rv-faq .item.open{border-color:#dcd0ad;box-shadow:0 14px 40px rgba(0, 0, 0,.10)}
#rv-faq .item:target{border-color:#F0C24A;box-shadow:0 0 0 3px rgba(240,194,74,.18)}
#rv-faq .item{position:relative}
#rv-faq .item .q,#rv-faq .item .a-wrap{position:relative;z-index:2}
#rv-faq .item::after{content:"";position:absolute;inset:0;border-radius:16px;pointer-events:none;opacity:0;transition:opacity .35s ease;z-index:1;background:radial-gradient(260px circle at var(--gx,50%) var(--gy,50%),rgba(240,194,74,.16),transparent 46%)}
...
#rv-faq .q::before{content:"";position:absolute;left:0;top:14px;bottom:14px;width:3px;border-radius:0 3px 3px 0;background:#F0C24A;transform:scaleY(0);transform-origin:center;transition:transform .3s ease}
#rv-faq .item.open .q::before{transform:scaleY(1)}
#rv-faq .q .ico{flex:0 0 28px;width:28px;height:28px;border-radius:50%;background:#f4eedd;color:#F0C24A;display:flex;align-items:center;justify-content:center;transition:transform .35s cubic-bezier(.5,.2,.2,1),background .25s}
.dark #rv-faq .q .ico{background:#2a2413}
#rv-faq .q .ico svg{width:15px;height:15px}
#rv-faq .item.open .q .ico{transform:rotate(135deg);background:#F0C24A;color:#fff}
#rv-faq .q .txt{flex:1}
...
#rv-faq .cta p{color:#D7D4D1;max-width:520px;margin:14px auto 0;line-height:1.6}
#rv-faq .btn{display:inline-flex;align-items:center;gap:9px;margin-top:26px;background:#F0C24A;color:#000000;font-weight:700;padding:14px 30px;border-radius:999px;text-decoration:none;transition:transform .25s,box-shadow .25s}
#rv-faq .btn:hover{transform:translateY(-2px);box-shadow:0 12px 26px rgba(240,194,74,.34)}
        ...
        if (fine && !reduce) {
            items.forEach(function(it){
                it.addEventListener('pointermove', function(e){
                    ...
                    it.style.boxShadow = (-px*16).toFixed(1) + 'px ' + (12 - py*10).toFixed(1) + 'px 34px rgba(0, 0, 0,.15)';
