...
<main class="flex-grow">
    <section class="relative py-24 text-center px-6">
        <div class="max-w-3xl mx-auto">
            ...
            <span class="block text-xs font-bold text-secondary-ink dark:text-secondary uppercase tracking-[0.3em] mb-3"><?php echo esc_html( romvill_t( 'ana.dim' ) ); ?> 03</span>
            <h1 class="text-4xl md:text-6xl font-serif font-bold text-slate-900 dark:text-white mb-6"><?php echo esc_html( romvill_t( 'perfil.san.h1' ) ); ?></h1>
    ...
    <section class="max-w-5xl mx-auto px-6 pb-24">
        <div class="grid md:grid-cols-2 gap-8">
            <?php
            $cards = array(
                array('icon'=>'plus-square','title'=>romvill_t('perfil.san.c1.title'),'desc'=>romvill_t('perfil.san.c1.desc')),
                array('icon'=>'clipboard-check','title'=>romvill_t('perfil.san.c2.title'),'desc'=>romvill_t('perfil.san.c2.desc')),
                array('icon'=>'pill','title'=>romvill_t('perfil.san.c3.title'),'desc'=>romvill_t('perfil.san.c3.desc')),
                array('icon'=>'users','title'=>romvill_t('perfil.san.c4.title'),'desc'=>romvill_t('perfil.san.c4.desc')),
            );
            foreach ($cards as $c) :
            ?>
        ...
        <div class="mt-16 text-center">
            <a href="<?php echo esc_url($contacto_url); ?>" class="inline-flex items-center gap-2 bg-secondary hover:bg-[#B5952C] text-slate-900 px-8 py-4 rounded-lg font-bold shadow-lg shadow-secondary/20 hover:shadow-xl transition-all">
                <?php echo esc_html( romvill_t( 'perfil.san.cta' ) ); ?> <span aria-hidden="true" class="material-symbols-outlined text-sm">arrow_forward</span>
            </a>
        </div>
    </section>
