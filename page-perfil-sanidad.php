...
<main class="flex-grow">
    ...
    <?php romvill_perfil_viz( 'sanidad' ); ?>

    <section class="max-w-5xl mx-auto px-6 pb-24">
        <div class="grid md:grid-cols-2 gap-8">
            <?php
            $cards = array(
                array('icon'=>'plus-square','title'=>'Red hospitalaria','desc'=>'Distancia y tiempo de acceso a los hospitales más cercanos (públicos y privados), especialidades disponibles y nivel de saturación.'),
                array('icon'=>'clipboard-check','title'=>'Atención primaria','desc'=>'Centros de salud asignados, disponibilidad de citas, ratio médico-paciente y valoración de los profesionales.'),
                array('icon'=>'pill','title'=>'Farmacias y urgencias','desc'=>'Farmacias de guardia, servicios de urgencias 24h cercanos y tiempo estimado de respuesta de emergencias.'),
                array('icon'=>'users','title'=>'Servicios especializados','desc'=>'Centros geriátricos, rehabilitación, pediatría especializada y otros servicios médicos relevantes para el perfil del residente.'),
            );
            foreach ($cards as $c) :
            ?>
        ...
        <div class="mt-16 text-center">
            <a href="<?php echo esc_url($contacto_url); ?>" class="inline-flex items-center gap-2 bg-secondary hover:bg-[#a3884c] text-slate-900 px-8 py-4 rounded-lg font-bold shadow-lg shadow-secondary/20 hover:shadow-xl transition-all">
                Solicitar Análisis Sanitario <span aria-hidden="true" class="material-symbols-outlined text-sm">arrow_forward</span>
            </a>
        </div>
    </section>
    <?php romvill_related_dimensions( 'perfil-sanidad' ); ?>
</main>
